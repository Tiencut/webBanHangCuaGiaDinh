...
const authAPI = {
        ...
        // Refresh token
        refreshToken: (refreshToken) => api.post('/auth/refresh-token', null, { params: { refreshToken } }),
        ...
        forgotPassword: (email, frontendUrl) => api.post(`/auth/forgot-password?email=${encodeURIComponent(email)}${frontendUrl ? `&frontendUrl=${encodeURIComponent(frontendUrl)}` : ''}`),
  ...
  resetPassword: (token, newPassword) => api.post(`/auth/reset-password?token=${encodeURIComponent(token)}&newPassword=${encodeURIComponent(newPassword)}`)
        ...
        validateResetToken: (token) => api.get(`/auth/validate-reset?token=${encodeURIComponent(token)}`)
}
...
// Draft Orders API Service
const draftsAPI = {
  // Save a new draft (server will attach user)
  save: (draft) => api.post('/orders/drafts', draft),
  // Get drafts for current user
  getAll: () => api.get('/orders/drafts'),
  // Get draft by id
  getById: (id) => api.get(`/orders/drafts/${id}`),
  // Update draft
  update: (id, draft) => api.put(`/orders/drafts/${id}`, draft),
  // Delete draft
